...
export type CountryListQuery = (
  ...
  & { Country?: Maybe<Array<Maybe<(
    { __typename?: 'Country' }
    & Pick<Country, 'name' | 'nativeName' | 'alpha2Code' | 'alpha3Code' | 'area' | 'population' | 'populationDensity' | 'capital' | 'demonym' | 'gini' | 'numericCode'>
    & { location: (
      { __typename?: '_Neo4jPoint' }
      & Pick<_Neo4jPoint, 'latitude' | 'longitude'>
    ), subregion?: Maybe<(
      { __typename?: 'Subregion' }
      & Pick<Subregion, 'name'>
      & { region?: Maybe<(
      ...
      { __typename?: 'Language' }
      & Pick<Language, 'iso639_1' | 'iso639_2' | 'name' | 'nativeName'>
    )>>>, currencies?: Maybe<Array<Maybe<(
      { __typename?: 'Currency' }
      & Pick<Currency, 'name' | 'symbol'>
    )>>>, regionalBlocs?: Maybe<Array<Maybe<(
      { __typename?: 'RegionalBloc' }
      & Pick<RegionalBloc, 'name' | 'acronym'>
      & { otherAcronyms?: Maybe<Array<Maybe<(
        { __typename?: 'OtherAcronym' }
        & Pick<OtherAcronym, 'name'>
      )>>>, otherNames?: Maybe<Array<Maybe<(
        { __typename?: 'OtherName' }
        & Pick<OtherName, 'name'>
      )>>> }
    )>>>, flag?: Maybe<(
      { __typename?: 'Flag' }
      & Pick<Flag, 'emoji' | 'emojiUnicode' | 'svgFile'>
    )>, topLevelDomains?: Maybe<Array<Maybe<(
      { __typename?: 'TopLevelDomain' }
      & Pick<TopLevelDomain, 'name'>
    )>>>, callingCodes?: Maybe<Array<Maybe<(
      { __typename?: 'CallingCode' }
      & Pick<CallingCode, 'name'>
    )>>>, alternativeSpellings?: Maybe<Array<Maybe<(
      { __typename?: 'AlternativeSpelling' }
      & Pick<AlternativeSpelling, 'name'>
    )>>> }
  )>>> }
);
...
export const CountryListComponent = (props: Omit<Urql.QueryProps<CountryListQuery, CountryListQueryVariables>, 'query'> & { variables?: CountryListQueryVariables }) => (
  <Urql.Query {...props} query={CountryListDocument} />
);


export function useCountryListQuery(options: Omit<Urql.UseQueryArgs<CountryListQueryVariables>, 'query'> = {}) {
  return Urql.useQuery<CountryListQuery>({ query: CountryListDocument, ...options });
};